...
class ThrottlingRateLimiter:
    ...
    def can_send_message(self, user_id: str) -> bool:
        ...
        if user_id not in self.user_last_message_time:
            return True
        return current_time - self.user_last_message_time[user_id] >= self.min_interval
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        ...
        if user_id not in self.user_last_message_time:
            return 0.0
        return max(0.0, self.min_interval - (current_time - self.user_last_message_time[user_id]))
