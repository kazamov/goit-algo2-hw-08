...
class SlidingWindowRateLimiter:
    ...
    def _cleanup_window(self, user_id: str, current_time: float) -> None:
        if user_id not in self.user_message_times:
            return
        while self.user_message_times[user_id] and self.user_message_times[user_id][0] <= current_time - self.window_size:
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        current_time = time.time()
        self._cleanup_window(user_id, current_time)
        if user_id not in self.user_message_times or not self.user_message_times[user_id]:
            return 0.0
        return max(0.0, self.window_size - (current_time - self.user_message_times[user_id][0]))
